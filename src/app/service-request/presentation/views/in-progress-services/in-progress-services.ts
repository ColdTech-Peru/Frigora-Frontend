...
export class InProgressServicesComponent implements OnInit, OnDestroy {
  ...
  async fetchData(): Promise<void> {
    ...
    try {
      const [accepted, inProgress, techs]: any = await Promise.all([
        firstValueFrom(this.serviceRequestApi.getRequestsForProviderQuery(this.currentProviderId, 'accepted')),
        firstValueFrom(this.serviceRequestApi.getRequestsForProviderQuery(this.currentProviderId, 'inProgress')),
        firstValueFrom(this.techniciansApi.getTechniciansByProvider(this.currentProviderId))
      ]);
      ...
      this.activeRequests = allRequests.map((r: any) => ({
        ...r,
        technicianName: this.technicians.find(t => String(t.id) === String(r.technicianId))?.name ?? null
      }));
  ...
  async assignTechnician(requestId: string | number): Promise<void> {
    ...
    try {
      await firstValueFrom(this.serviceRequestApi.sendAssignTechnicianCommand(requestId, techId));
  ...
  async completeService(requestId: string | number): Promise<void> {
    try {
      await firstValueFrom(this.serviceRequestApi.sendCompleteRequestCommand(requestId));
      await this.fetchData();
    } catch (err) {
